...
| 14         | `apps/server/src/config.ts`, `apps/server/.env.example`, `apps/server/src/e2e/fixture-asset.ts`, `apps/server/src/e2e/fixture-seeder.ts`, `apps/server/src/e2e/fixture-cli.ts`, `apps/server/src/e2e/fixture-cli.test.ts`, `apps/web/e2e/run.mjs`, `apps/web/e2e/lib/bridge.mjs`, `apps/web/e2e/lib/bridge.test.mjs`, `apps/web/e2e/lib/env.mjs`, `apps/web/e2e/lib/manifest.mjs`, `apps/web/e2e/lib/manifest.test.mjs`, `apps/web/e2e/fixtures/seed.mjs`, `apps/web/e2e/cases/design-system-regression.md`, `apps/web/e2e/suites/design-system-regression.mjs`, `apps/web/e2e/baselines/manifest.json`, `apps/web/e2e/baselines/design-lab/light/390.png`, `apps/web/e2e/baselines/design-lab/light/1024.png`, `apps/web/e2e/baselines/design-lab/light/1440.png`, `apps/web/e2e/baselines/design-lab/light/1895.png`, `apps/web/e2e/baselines/design-lab/dark/390.png`, `apps/web/e2e/baselines/design-lab/dark/1024.png`, `apps/web/e2e/baselines/design-lab/dark/1440.png`, `apps/web/e2e/baselines/design-lab/dark/1895.png`, `apps/web/e2e/baselines/feed-home/light/390.png`, `apps/web/e2e/baselines/feed-home/light/1024.png`, `apps/web/e2e/baselines/feed-home/light/1440.png`, `apps/web/e2e/baselines/feed-home/light/1895.png`, `apps/web/e2e/baselines/feed-home/dark/390.png`, `apps/web/e2e/baselines/feed-home/dark/1024.png`, `apps/web/e2e/baselines/feed-home/dark/1440.png`, `apps/web/e2e/baselines/feed-home/dark/1895.png`, `apps/web/e2e/baselines/chain-home/light/390.png`, `apps/web/e2e/baselines/chain-home/light/1024.png`, `apps/web/e2e/baselines/chain-home/light/1440.png`, `apps/web/e2e/baselines/chain-home/light/1895.png`, `apps/web/e2e/baselines/chain-home/dark/390.png`, `apps/web/e2e/baselines/chain-home/dark/1024.png`, `apps/web/e2e/baselines/chain-home/dark/1440.png`, `apps/web/e2e/baselines/chain-home/dark/1895.png`, `apps/web/e2e/README.md` | 15                       |
...
```bash
git add apps/web/src/ui/floating/FloatingLayer.tsx apps/web/src/ui/floating/FloatingLayer.test.tsx apps/web/src/ui/menu/Menu.tsx apps/web/src/ui/menu/Menu.test.tsx apps/web/src/ui/menu/index.ts apps/web/src/ui/popover/Popover.tsx apps/web/src/ui/popover/index.ts apps/web/src/ui/tooltip/Tooltip.tsx apps/web/src/ui/tooltip/index.ts
...
- Produces: config-owned `MOMENT_E2E`, `MOMENT_E2E_OWNER_EMAIL`, `MOMENT_E2E_OWNER_PASSWORD`, `MOMENT_E2E_VIEWER_EMAIL`, and `MOMENT_E2E_VIEWER_PASSWORD`; `assertE2eFixtureGuard(env: Pick<Config, 'MOMENT_E2E' | 'NODE_ENV' | 'MYSQL_DATABASE'>): void`, which permits fixture work only when `MOMENT_E2E === '1'`, `NODE_ENV === 'test'`, and `MYSQL_DATABASE === 'moment_e2e'`; `assertE2eStorageGuard(env: Pick<Config, 'ATTACHMENT_S3_BUCKET' | 'ATTACHMENT_S3_ENDPOINT' | 'ATTACHMENT_S3_IS_PUBLIC'>): void`, which additionally requires the private bucket name `moment-e2e` and an HTTP loopback S3/MinIO endpoint; and `readE2eFixtureCredentials(env: Pick<Config, 'MOMENT_E2E_OWNER_EMAIL' | 'MOMENT_E2E_OWNER_PASSWORD' | 'MOMENT_E2E_VIEWER_EMAIL' | 'MOMENT_E2E_VIEWER_PASSWORD'>): E2eFixtureCredentials`. `fixture-asset.ts` exports `FIXTURE_IMAGE_PNG: Buffer`, `FIXTURE_IMAGE_MIME: 'image/png'`, `FIXTURE_IMAGE_WIDTH: 64`, `FIXTURE_IMAGE_HEIGHT: 48`, and the adapter-relative `FIXTURE_IMAGE_STORAGE_KEY: 'chains/00000000-0000-4000-8000-000000000014/00000000-0000-4000-8000-000000000016/00000000-0000-4000-8000-000000000017.png'`. `fixture-seeder.ts` exports `resetFixture(): Promise<{ ok: true }>`, `seedFixture(credentials: E2eFixtureCredentials): Promise<DesignSystemFixture>`, `teardownFixture(): Promise<{ ok: true }>`, and the narrow `closeFixtureDb(): Promise<void>` that calls the existing `pool.end()` exactly once per write-action process. `runFixtureAction(action: 'preflight' | 'reset' | 'seed' | 'teardown', dependencies?: { loadSeeder(): Promise<FixtureSeederModule>; serialize(value: E2eCliResult): string }): Promise<string>` dynamically imports the seeder and uses `JSON.stringify` by default, while the pure contract test injects both dependencies. `FixtureSeederModule` exposes exactly the four fixture-seeder functions above. `preflight` returns the serialized exact value `{ mode: 'e2e', database: 'moment_e2e' }` without calling `loadSeeder`; each write action performs action → serialization → `closeFixtureDb()` in `finally`, and any action, serialization, or closer failure yields a nonzero CLI exit without `process.exit()`, credential text, or MySQL fields. `seed` returns password-free `DesignSystemFixture = { owner: { email }, viewer: { email }, chainId, momentId, imageMomentId, mediaId, shareToken, inviteToken, fixedNow, apiBaseUrl, webBaseUrl }`; `reset` and `teardown` return `{ ok: true }`. Web produces `node e2e/run.mjs design-system-regression [--update-baselines]`; `seed({ action: 'reset' | 'seed' | 'teardown' }): Promise<DesignSystemFixture | { ok: true }>`; `assertE2eEnvironment(): WebE2eEnvironment`, whose owner/viewer passwords come directly from the runner's local environment and are never accepted from CLI stdout; `bridge.request(action: string, args: object): Promise<unknown>`, which sends `{ action, args, session: 'e2e-web-design-system-refactor' }`, requires `success === true`, and returns only `data`; `bridge.comparePng({ baselinePath, actualPath, threshold: 0.1, maxDiffPixels: 120 }): Promise<{ diffPixels: number; diffPath: string }>`; and `loadBaselineManifest(): Promise<BaselineCase[]>`, where every case has `requiredContent: Array<'大家的日子' | '单链页' | '纯文字时刻' | '单图时刻' | '跨年索引' | '长Tag' | '长链名'>`, plus `node apps/web/e2e/lib/manifest.mjs --verify|--hashes`. The sole manifest owns exactly the 24 PNG files listed in this Task's Files; only `--update-baselines` may write one of them, while a normal run writes actual/diff/log evidence only under ignored `apps/web/e2e/artifacts/**`.

- [ ] **Step 1: Write the failing guarded-CLI contract and natural-language case before automation.**

Create `apps/server/src/e2e/fixture-cli.test.ts` as Node's built-in test-runner unit test. Before any implementation, it must import the future guard/credential/CLI parsing exports and assert that each absent or invalid member of the triple database guard fails before a seeder method can run: `MOMENT_E2E !== '1'`, `NODE_ENV !== 'test'`, and a missing or non-exact `MYSQL_DATABASE` (including another `_e2e` database name). Assert the storage guard rejects a non-`moment-e2e` bucket, a missing/non-loopback endpoint, or a public bucket, without contacting S3. Assert each missing/empty/invalid E2E email or password is rejected before `seed`, while `preflight`, `reset`, and `teardown` do not require credentials. Assert `preflight` serializes exactly `{ mode: 'e2e', database: 'moment_e2e' }` and never calls the injected `loadSeeder`; `seed` stdout omits both password keys and password values; unsupported actions are rejected; and error text contains neither supplied credentials nor MySQL connection fields. Inject fake seeder methods, `closeFixtureDb` and serializer, record call order, and assert the successful path records `action, serialize, close`, the throwing-action path records `action, close`, and the throwing-serializer path records `action, serialize, close`; all three call the closer exactly once and never before the action settles. Assert the executable sets `process.exitCode` rather than calling `process.exit`. This test must neither import `fixture-seeder.ts`/`db`, open a pool, call `resetDb()`, run migrations, contact S3, or use Jest.
...
Passwords come only from ignored local `apps/server/.env.e2e` or CI's non-production secret store: do not put a password, access token, or MySQL connection field in any tracked fixture, case, suite, screenshot name, terminal output, or README. The case says to first reset then seed; to visit `/__design-lab`, `/`, `/chains/{chainId}`, `/chains/{chainId}?compose=1`, `/chains/{chainId}/settings`, `/moments/{momentId}`, `/me`, `/notifications`, `/share/{shareToken}`, `/invites/{inviteToken}`, `/login`, `/register`, and authenticated wildcard; and to verify the visible role/read-only/invite-from, reaction/comment/edit/delete, tag/order/date-anchor, loading/error/empty, and no-business-semantics-drift states. It names and visibly verifies the long chain `我们一起走过的很长很长的时光链名字`, the long tag `跨年旅行与新年第一束光和家人的漫长回忆`, the pure-text moment `2025 年最后一天：一起把这一年的温柔收好。` at `2025-12-31T15:30:00.000Z`, the one-image moment `2026 年第一天：新年的第一束光。` at `2026-01-01T00:30:00.000Z`, and the 2025/2026 rail boundary. Both `/` and `/chains/{chainId}` must show both seeded moments, the image, long values and cross-year index together before their screenshot; `/` additionally shows `大家的日子`, and the chain route proves the single-chain variant. Cover light and dark at 390×844, 1024×900, 1440×900 and 1895×900; keyboard and overlay behavior; reduced motion; and a separate 1440×900 light-theme 200% zoom journey with visible labels and no horizontal clipping.
...
`fixture-cli.ts` statically imports only `config` and pure CLI helpers, accepts exactly one action argument, validates both the triple database guard and the non-production storage guard, and reports a guard/action/seeder/serialization/close error by setting `process.exitCode = 1` after writing a fixed sanitized error; it never calls `process.exit()`. The database guard requires the entire database name to equal `moment_e2e`; an `_e2e` marker in another MySQL field or a different suffixed database is rejected. The storage guard parses `ATTACHMENT_S3_ENDPOINT` with `URL`, accepts only `http:` plus hostname `127.0.0.1`, `localhost` or `[::1]`, requires private `ATTACHMENT_S3_BUCKET === 'moment-e2e'`, and rejects `ATTACHMENT_S3_IS_PUBLIC !== false`; it performs no network request. `preflight` is guard-only and reports the fixed JSON contract without importing `fixture-seeder.ts`, so it opens neither MySQL nor S3. Only `seed` calls `readE2eFixtureCredentials(config)`; the other actions do not require credentials. For `reset|seed|teardown`, `runFixtureAction` dynamically obtains the module from its injected/default `loadSeeder`, awaits the selected method, calls the injected/default serializer while the DB is still open, and then always awaits `closeFixtureDb()` exactly once in `finally`; a closer error remains fatal. This ordering is the contract that lets `child_process.execFile` observe real process termination on success, action failure and JSON-serialization failure.
...
`fixture-seeder.ts` is the only DB/storage-writing producer. It accepts credentials already parsed from `config` and never reads `process.env`. `resetFixture` obtains the current adapter and snapshot through `getStorage()` and `currentStorageMeta()`, deletes `FIXTURE_IMAGE_STORAGE_KEY` with `storage.deleteFile(key, storageMeta)`, then deletes the existing schema rows in foreign-key reverse order (`pushTokens`, `notifications`, `reactions`, `comments`, `momentTags`, `tags`, `outbox`, `media`, `moments`, `chainInvites`, `chainMembers`, `shareLinks`, `chains`, `refreshTokens`, `users`) and returns only after both storage and DB are empty. S3/MinIO `DeleteObject` is idempotent, so a missing fixture object is success. `seedFixture` first awaits that reset, then calls the existing `UnifiedStorageAdapter.uploadFile(FIXTURE_IMAGE_STORAGE_KEY, FIXTURE_IMAGE_PNG)`—the current abstraction derives the object path from configured bucket/prefix and accepts no MIME argument—and captures `currentStorageMeta()` for the media row. It transactionally inserts the two configured E2E users, owner/viewer chain members, one chain named `我们一起走过的很长很长的时光链名字`, the pure-text moment `00000000-0000-4000-8000-000000000015` at `2025-12-31T15:30:00.000Z`, the media moment `00000000-0000-4000-8000-000000000016` at `2026-01-01T00:30:00.000Z`, and media `00000000-0000-4000-8000-000000000017` with `momentId` set to the media moment, `s3Key: FIXTURE_IMAGE_STORAGE_KEY`, `mime: FIXTURE_IMAGE_MIME`, `size: FIXTURE_IMAGE_PNG.byteLength`, fixed 64×48 dimensions, `sortOrder: 0`, `status: 'ready'`, and `storageMeta` set to that snapshot. Both moments use `happenedTzOffset: -480`; the text moment has `type: 'text'`, the image moment has `type: 'media'`, and their exact visible contents are the Step 1 strings. Insert the tag `跨年旅行与新年第一束光和家人的漫长回忆` and associate it with the text moment, plus the share link and pending invite using the exact stable IDs/tokens from Step 1. These owner-membership rows make both moments visible on `/` and `/chains/{chainId}`. If the DB transaction fails after upload, catch the error, delete the just-uploaded object with the same snapshot metadata, and rethrow. `teardownFixture` delegates to `resetFixture`. `closeFixtureDb` imports the existing `pool` from `src/db/index.ts` and only awaits `pool.end()`; no other task or caller accesses the pool directly. Do not create an HTTP endpoint, controller, Express route, middleware exception, reset secret, direct `S3Client`, or second storage adapter.
...
Expected: PASS; the server test does not import `db`, open a pool, call `resetDb()`, run migrations, invoke `pnpm --filter @moment/server test`, or invoke repository-wide `pnpm test`. Its injected success, action-failure and JSON-serialization-failure traces each call `closeFixtureDb` exactly once, after the action and after any attempted serialization, and `preflight` never loads or closes the DB module. The bridge test uses only its fetch stub and proves the action/args/data contract without contacting CSI or navigating Chrome.
...
git add apps/server/src/config.ts apps/server/.env.example apps/server/src/e2e/fixture-asset.ts apps/server/src/e2e/fixture-seeder.ts apps/server/src/e2e/fixture-cli.ts apps/server/src/e2e/fixture-cli.test.ts
...
Expected: every command exits 0; the server-side CLI check remains a pure no-database Node test (no `resetDb()`, migration, `pnpm --filter @moment/server test`, or repository-wide `pnpm test`) and proves success/action-failure/serialization-failure each close exactly once after the final DB-dependent work; the bridge test proves CSI uses action/args/data without contacting the daemon; the manifest test proves exactly 24 complete PNGs and all seven required-content labels; and the Web build proves Task 1’s `react-dom/client` link fix remains valid.

- [ ] **Step 2: Run the full replayable visual/interaction matrix.**

Run: `pnpm --filter @moment/web e2e:design-system`

Expected: exit 0 for the manifest's `design-lab`, `feed-home` and `chain-home` baselines at 390/1024/1440/1895 in light/dark; the feed/chain captures visibly contain the seeded pure-text moment, decoded one-image moment, 2025/2026 index, long Tag and long chain name, with feed additionally showing `大家的日子` and chain proving the single-chain page. The full route journey, reduced motion, Tab/Shift+Tab, Escape, outside interactions, Modal/Sheet/AlertDialog/Menu/Popover/Tooltip layering, 767px ActionSheet/768px anchored behavior, resize-close/focus restoration, and documented 200% zoom evidence also pass.
...
- [ ] Before handoff, run `pnpm exec prettier --check docs/superpowers/plans/2026-08-18-web-design-system-refactor.md`, scan the plan for prohibited placeholders and retired CSI/DB/baseline forms, inspect all Task Files for unique ownership and producer-before-consumer ordering, run the structure and exact 24-PNG/required-content/pool-lifecycle/internal-export checks, and run `git diff --check`.
