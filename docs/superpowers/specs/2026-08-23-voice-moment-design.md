...
**moments 表**新增两列（`drizzle-kit generate` 出迁移，不手写 SQL）：
...
export const AUDIO_MIME_TYPES = [
  'audio/mp4',   // m4a / AAC（app 端 expo-av 默认产物）
  'audio/x-m4a',
  'audio/aac',
  'audio/mpeg',  // mp3
...
`packages/api-client` 随 dto 类型自动获得新字段（typed fetch 无手写类型副本）；若有 media presign 的封装函数签名引用 `kind` 联合类型，同步加 `'audio'`。
...
- 媒体行 mime 校验矩阵扩展（现有 `video` / `media` 分支不动）：

| type | 校验 |
|---|---|
| `voice` | `mediaRows` 中**恰好 1 条 `audio/*`**，其余全部 `image/*`；任一违反 → 400 `MEDIA_INVALID` |
...
3. `getASRProvider()` 为 `null` → 置 `failed` 后正常返回（部署方停用转写，不占重试额度）。
4. 生成内部预签名 GET（复用 `getStorage().generateAccessUrl`，短 TTL）→ 下载字节（响应 size > `MAX_AUDIO_BYTES` 直接置 `failed`，防御行上 size 与对象不符）。
5. 调 `transcribe()`：
   - **成功**：单条 UPDATE 落 `transcript = text`、`transcriptionStatus = 'done'`，且 **`content` 条件回填**——`SET content = text WHERE id = ? AND content = ''`（用户可能在转写完成前已手动编辑正文，不覆盖用户输入；回填与状态更新同事务/同语句批，避免中间态）。空文本（笑声/环境音）也置 `done`，`transcript` 存空串。
...
outbox 重试耗尽（5 次退避后标 failed）后 moment 会挂在 `pending`。sweeper 增加一路：voice moment `transcription_status = 'pending'` 且 `created_at` 超过 1 小时 → 置 `failed`。与现有 sweep 任务同周期执行；这路 sweep 同时覆盖「outbox 行丢失/ worker 长期宕机」的极端场景。
...
- **录制入口**：`apps/app/src/features/compose/` 新增 voice 类型。录音用 `expo-av`（`Audio.Recording`，预设 m4a/AAC，`audio/mp4` 在白名单内）；需处理麦克风权限申请与拒绝态。
- **录制交互**与 web 对齐：时长显示、300s 自动停、回听、重录。
- **附图**：复用现有图片选择；合计 ≤ 9。
- **消费侧**：voice 卡片播放条用 `useMediaUri(audioMediaId)` / `fetchMediaBlob` 拿本地 uri 后 `expo-av` 播放（**不能用裸 url 直渲**——原生播放器不带鉴权头，与 video-poster §4 的约束相同）；三态文案规则同 web。
...
- **server**（`apps/server/tests/` 触库）：
  ...
  - sweeper：pending 超 1h 置 failed；
  - 软删带 audio 的 voice moment → audio 行随既有路径 orphaned。
- **server 测试夹具同步**：`MomentResponse` 新增两个必填字段后，手工构造该类型字面量的测试工厂/夹具必须补 `transcript` / `transcriptionStatus`（voice 场景除外一律给 `null`）；`MediaLike` 不变。web/app 侧同理（参照 video-poster §6 列出的夹具位置逐一排查，grep `: MomentResponse` 与手写 moment 字面量构造点）。
- **web / app**：`pnpm lint` + tsc；手测录制/转码/上传/降级（模拟 ASR 失败）与三态卡片渲染。
- **全量验证门槛**：`pnpm test` 通过。
