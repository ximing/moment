...
| Token | 浅色 | 深色 | 唯一语义 |
|---|---|---|---|
| `--bg` | `#f7f4ef` | `#1a1816` | 页面底(中性微暖纸,非奶油黄 / 酱油黑) |
| `--surface` | `#fffcf8` | `#26221d` | 卡片/面板 |
| `--ink` | `#1c1917` | `#f3eee6` | 主文字 |
| `--muted` | `#6b645c` | `#b0a89d` | 次要文字(两主题均达 AA) |
| `--line` | `#e4ddd4` | `#3d3832` | **结构线**:卡片、分割、输入框(1px) |
| `--stroke` | `#2a241c` | `#b8b0a4` | **性格线**:贴纸、按钮、链节、侧栏(2px;侧栏 1px) |
| `--shadow` | `rgba(28,25,23,.08)` | `rgba(0,0,0,.40)` | 偏移影颜色(几何见 §1.2) |
| `--action` | `#de4a28` | `#fb6a45` | **动作**:发布按钮、FAB、通知角标、主 CTA |
| `--action-fg` | `#ffffff` | `#1a1816` | 动作按钮上的字(深色提亮橙后改深墨,保证对比) |
| `--select` | `#f0b84a` | `#f0b84a` | **选中/热态**:当前导航、我点过的表情、日期贴纸(深色) |
| `--select-fg` | `#241a0b` | `#241a0b` | 黄底上的字,两主题恒深墨 |
| `--danger` | `#c93a2e` | `#ff8a66` | 危险操作 |
| 贴纸四色 | 粉 `#ffd9e6` / 蓝 `#cfe8ff` / 薄荷 `#bfe8d0` / 紫 `#e6dbff` | 粉 `#3f2c34` / 蓝 `#27364a` / 薄荷 `#264036` / 紫 `#332c48` | **仅装饰**,由 hash 轮换(见 §1.4),永不表意 |
| 贴纸描边 | 同 `--stroke` | 粉 `#c08497` / 蓝 `#7aa0c8` / 薄荷 `#6eae93` / 紫 `#9b8fd0` | 浅色走性格线;深色同色系中亮边 |

**色彩职责纪律(对抗审查 #5/#8 的修正,2026-08-17 纸感修订):**

- 橙=动作、黄=选中/热态、`--line`=结构、`--stroke`=性格描边;**禁止**再把浅色卡片描边绑到 `--ink`。
- 深色动作橙提到 `#fb6a45`,禁止把动作也压进黄色导致「当前导航」与「主按钮」同色。
- 每个颜色只有一行语义;新增用途先加 token,禁止组件内写死色值,禁止组件层按主题各自判断颜色。

### 1.2 形状语言

- 卡片/面板:圆角 16–18px,`1px solid var(--line)`,偏移影 `3px 3px 0 var(--shadow)`。
- 贴纸(标签/表情/日期/状态/导航热态):圆角 99px 胶囊,`2px solid var(--stroke)`(或贴纸四色 line),小一号偏移影 `2px 2px 0 var(--shadow)`。
- 按钮:主按钮橙底圆角 99px 描边 2px `--stroke`;次按钮 surface 底同描边;危险按钮 `--danger`。
- 媒体宫格:2px `--stroke` + 圆角 10–12px,格间距 4–6px。
- 时间线节奏:日期组 `mb-8`,组内卡片 `gap-4`,日期头下 `mb-4`(组间松、组内紧)。
...
- 左栏 232px:得意黑字标(「刻」用 `--action`)、当前项 `--select` 黄底贴纸态、链列表带 §1.4 颜色点、通知角标走 `--action` / `--action-fg`;侧栏分隔 `1px solid var(--stroke)`。
- 卡片列 `--content` 800px（时间线卡片列，照片为主）；Shell 主列不再套 `max-w-content`，各页自行约束。
- **≥1400px 右栏**(宽屏才出现):贴在卡片列外侧，不计入 `--content`。上「时间索引」(§4),下「筛选」(链多选 chips → `chain_ids`;标签单选 → `tag_id`;「按添加时间看补发」开关 → `order=created_at`)。
...
- **链设置**(`/chains/:id/settings`):顶栏贴纸 chips（分享/成员/资料）；危险区沉在资料页底；不要左目录。权限隐藏规则保留(viewer 只见成员只读;editor 无分享生成)。分享链接每行一张贴纸卡,状态色:有效=薄荷、已过期=黄、已吊销=灰;吊销二次确认文案沿用。「成员/资料」交互沿用 web-product §6。
