...
1. `/chains/:id` 标题右侧出现成员头像（含自己，不必先点设置）；超过 5 人时只画前 5 个加 `+N`。
2. 头像 hover（触控为点按）浮出昵称 + 角色（创建者 / 可记录 / 只看）。
3. `visibility === 'public'` 出「公开」；`link` 出「链接可看」；`private` 不画标识。
4. 6 人及以上时前 5 个头像 + `+N`，`N = memberCount - 5`。
...
| 字段 | 规则 |
|---|---|
| `membersPreview` | 该链全部成员的前 5 名。排序：`joinedAt` 升序，并列再按 `userId` 升序。含当前用户。不含待接受邀请。 |
| `memberCount` | 该链成员总数（含自己）。恒 `>= membersPreview.length`。 |
| 单条预览 | 昵称、头像 URL（无则 `null`）、角色。无邮箱、无 `joinedAt`。 |
...
**组装：** `ChainService` 在序列化前按本次涉及的 `chainId` **一次批量**查成员与头像，再挂到每条 `ChainDto`。`listMine` 禁止按链循环查成员。成员查询失败则整次 list/get/create/update/transfer 失败，禁止返回「链成功、`membersPreview: []` 且 `memberCount: 0`」的假空预览（新建链至少有创建者，`memberCount >= 1`）。
...
- 数据源：`chain.membersPreview`。渲染全部预览项（最多 5）。
...
`createInvite` 只产生待接受邀请，不改 `membersPreview`，不必为此新发事件。
...
**server**（真实测试库，`--runInBand`，`resetDb` + `afterAll(closeDb)`）：扩 `apps/server/tests/chains/chains.crud.test.ts` 与 `chains.members.test.ts`。

| 用例 | 断言 |
|---|---|
| `POST /chains` | `membersPreview` 仅创建者（`role: 'owner'`，`userId` 匹配），`memberCount === 1` |
| 再加入 editor、viewer 后 `GET /chains/:id` | 预览按 `joinedAt` 升序；三人角色正确；`memberCount === 3` |
| 同上后 `GET /chains` | 该链项同样带预览与人数 |
| 第 6 个成员加入后 `GET /chains/:id` | `membersPreview.length === 5`，`memberCount === 6` |
| `PATCH visibility: 'link'` | 响应 `visibility === 'link'`，预览仍在 |
| 非成员 `GET` | 仍 404 `CHAIN_NOT_FOUND`，不泄露预览 |
| 未登录 | 仍 401 |
