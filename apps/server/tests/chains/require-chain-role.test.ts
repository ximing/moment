...
async function harness(): Promise<Server> {
  const app = express();
...
describe('requireChainRole 中间件', () => {
  it('editor 成员放行并把角色挂到 request.chainRole', async () => {
    ...
    const res = await request(await harness()).get('/x/chain-1').set('Authorization', `Bearer ${token}`);
  ...
  it('链不存在 → 404 CHAIN_NOT_FOUND', async () => {
    ...
    const res = await request(await harness()).get('/x/no-such-chain').set('Authorization', `Bearer ${token}`);
    expect(res.status).toBe(404);
    expect(res.body.error.code).toBe('CHAIN_NOT_FOUND');
  });

  it('无 token → 401 UNAUTHORIZED', async () => {
    const res = await request(await harness()).get('/x/chain-1');
